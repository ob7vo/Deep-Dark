...
std::pair<UnitAnimationState, bool> get_unit_ani_state(std::string_view str) {
    ...
    case 'p': 
        if (str == "phase_windup") return { UnitAnimationState::PHASE_WINDUP, false };
        else if (str == "phase_winddown") return { UnitAnimationState::PHASE_WINDDOWN, false };
        else throw std::runtime_error("unknown p-animation: " + std::string(str));
...
AnimationClip AnimationClip::from_json(const nlohmann::json& file, sf::Texture* pTexture, bool loops) {
    int frames = file["frames"].get<int>();

    // This is more lines, thohg it is more readable;
    float rate = 0.1f;
    if (file.contains("fps")) rate = 1.f / file["fps"].get<float>();
    else if (file.contains("duration")) rate = file["duration"].get<float>() / (float)frames;
    
    sf::Vector2i cellSizes = { file["cell_size"][0].get<int>(), file["cell_size"][1].get<int>() };
    sf::Vector2f origin = { file["origin"][0].get<float>(), file["origin"][1].get<float>() };
...
void AnimationClip::setup_unit_animation_map(const json& unitFile, UnitAnimMap& aniMap,
    ...
    for (const auto& [animName, animData] : unitFile["animations"].items()) {
        ...
        std::string path = UnitConfig::getUnitFolderPath(unitFile["unit_id"].get<int>());
