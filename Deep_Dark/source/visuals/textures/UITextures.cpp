...
namespace Textures::UI {
	...
	std::array<sf::Texture, UnitConfig::TOTAL_PLAYER_UNITS> t_unitSlots;

	const sf::Texture& getUnitSlot(int id) {
			if (id <= -1 || id >= 100) return t_defaultUnitSlot;
			else return t_unitSlots[id];
	}
	...
	void initialize() {
		...
		for (int id = 0; id < UnitConfig::TOTAL_PLAYER_UNITS; id++)
			t_unitSlots[id] = UnitConfig::createSlotTexture(id);
	}
}
