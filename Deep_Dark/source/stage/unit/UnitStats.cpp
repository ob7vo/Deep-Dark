...
UnitType UnitStats::convert_string_to_type(std::string_view strView) {
...
void UnitStats::setup(const nlohmann::json& file) {
    ...
    for (std::string& type : file["types"].get<std::vector<std::string>>())
        unitTypes |= convert_string_to_type(type);
    if (file.contains("target_types"))
        for (std::string& target : file["target_types"].get<std::vector<std::string>>())
            targetTypes |= convert_string_to_type(target);
    if (file.contains("immunities"))
        for (std::string& immunity : file["immunities"].get<std::vector<std::string>>())
            immunities |= Augment::string_to_augment_type(immunity);
    if (file.contains("augments_mask"))
        for (std::string& quick : file["augments_mask"].get<std::vector<std::string>>())
            augmentsMask |= Augment::string_to_augment_type(quick);

    if (file.contains("augments")) {
        for (auto& augJson : file["augments"]) {
            AugmentType augType = Augment::string_to_augment_type(augJson["augment"].get<std::string>());
    ...
    if (!file["stats"].contains("hits")) {
        ...
        for (auto& hit : file["stats"]["hits"]) {
            int dmg = hit["dmg"].get<int>();
            std::pair<int, int> laneReach = hit.value("lane_reach", baseRange);
            std::pair<float, float> attackRange = hit["attack_range"].get<std::pair<float,float>>();
            hits.emplace_back(dmg, laneReach, attackRange);
        }
...
UnitStats UnitStats::create_player(const nlohmann::json& file, int core) {
    ...
    if (file.contains("cores") && core >= 0)
        for (const std::string& coreStr : file["cores"][core])
            stats.apply_core_modifier(coreStr);
...
UnitStats UnitStats::create_cannon(const nlohmann::json& baseFile, float magnification) {
    ...
    stats.targetTypes = convert_string_to_type(baseFile["target_type"].get<std::string>());
...
void UnitStats::apply_core_modifier(const std::string& coreStr) {
    ...
    if (it != statModifiers.end()) {
        ...
        case '+': addCoreAugment(UnitConfig::createUnitJson(id), value); break;
        default:
...
void UnitStats::addCoreAugment(const nlohmann::json& file, const std::string& augStr) {
    ...
    for (auto& coreAug : file["core_augments"]) {
        AugmentType augType = Augment::string_to_augment_type(coreAug["augment"].get<std::string>());
