...
void Teleporter::action(Stage& stage) {
	...
	if (team == 0) {
		check_units(stage, stage.lanes[laneIdx].getAllyUnits(UnitConfig::ENEMY_TEAM));
		check_units(stage, stage.lanes[laneIdx].getAllyUnits(UnitConfig::PLAYER_TEAM));
	}
	else check_units(stage,stage.lanes[laneIdx].getAllyUnits(team));
}
