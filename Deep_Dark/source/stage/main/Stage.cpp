...
Unit* Stage::create_unit(int laneIndex, const UnitStats* unitStats, UnitAniMap* aniMap) {
	...
	if (auto index = unitPool.spawn_unit()) {
		...
		try_create_ability_observer(*index, nextUnitID);
...
void Stage::try_revive_unit(UnitSpawner* spawner) {
	...
	if (newUnit) {
		...
		newUnit->status.met_knockback_threshold(newUnit->stats->maxHp);
...
void Stage::transform_unit(const Unit& unit) {	
	...
	if (auto transformation = get_unit_config(aug.data.transform.ID, unit.stats->magnification)) {
		...
		if (transformedUnit) {
			transformedUnit->movement.pos = unit.movement.pos;
...
void Stage::create_summon(const Unit& unit) {
	...
	if (auto summonData = get_unit_config(aug.data.killStreak.salvageID, unit.stats->magnification)) {
		...
		if (summonedUnit) {
			summonData->count++;
			summonedUnit->movement.pos = { newX, unit.movement.pos.y };
...
void Stage::try_create_ability_observer(size_t poolIndex, int spawnIndex) {
	const UnitStats* stats = getUnit(poolIndex).stats;
...
void Stage::create_projectile(const Unit& unit, const Augment& aug) {
	...
	case 'p': {
		...
		proj.pathing = std::make_unique<ProjectilePathing>(unit.movement.pos, velocity, mass);
		break;
	}
	case 'w': {
		std::vector<sf::Vector2f> waypoints = {};
		for (const auto& wp : projJson["pathing"]["waypoints"])
			waypoints.push_back(unit.movement.pos + sf::Vector2f(wp["x"], wp["y"]));
