...
void StagePreviewMenu::setup_menu(int stage, int phase, Loadout* usedLoadout) {
	...
	for (int i = 0; i < 10; i++) {
		int usedUnitID = usedLoadout->slots[i].unitStats.id;

		usedUnitsSlotSprites[i].setTexture(getUnitSlot(usedUnitID), true);
		if (usedUnitID != -1) usedUnits[usedUnitID] = true;
	}
...
void StagePreviewMenu::create_enemy_sprites(const nlohmann::json& stageSetJson) {
	...
	for (const auto& enemy : stageSetJson["enemy_spawns"]) {
		...
		const nlohmann::json idleAnimConfig = UnitConfig::createUnitJson(id)["animations"]["idle"];
		std::string idleAnimPath = UnitConfig::getUnitFolderPath(id) + "idle.png";
