...
ArmoryMenu::ArmoryMenu(Camera& cam) : Menu(cam), stagePreviewMenu(cam) {
	...
	for (int id = 0; id < TOTAL_PLAYER_UNITS; id++) {
		inventorySlotBtn(id).setup(UI::ZERO, INVENTORY_SLOT_SCALE, Textures::UI::getUnitSlot(id));
	}
...
void ArmoryMenu::on_enter() {
	...
	for (int id = 0; id < TOTAL_PLAYER_UNITS; id++) {
		inventorySlotBtn(id).visible = UnitSaveData::IsUnlocked(id);

		if (UnitSaveData::IsUnlocked(id)) {
			inventorySlotBtn(id).set_texture(Textures::UI::getUnitSlot(id));
...
void ArmoryMenu::reset_positions() {
	...
	for (int id = 0; id < TOTAL_PLAYER_UNITS; id++) {
		if (!UnitSaveData::IsUnlocked(id)) continue;
...
void ArmoryMenu::drag_unit_into_slot() {
	...
	for (int i = 0; i < EQUIP_SLOTS; i++) {
		if (equipSlots[i].hovered_over(mPos)) {
			equipSlots[i].set_unit(curHeldUnitID, !unit_is_unusable(curHeldUnitID));
			shift_empty_slots();
...
void ArmoryMenu::start_dragging_unit(int id) {
	...
	if (unit_is_unusable(id)) {
		std::cout << "unusable" << std::endl;
		return;
	}

	curHeldUnitID = id;
	cam.set_cursor_ui(Textures::UI::getUnitSlot(curHeldUnitID), DRAG_SLOT_ORIGIN, DRAG_SLOT_OPACITY, INVENTORY_SLOT_SCALE);
}
...
void ArmoryMenu::update_display_of_unit(int id) {
	...
	for (int i = 0; i < filledUnitSlots; i++)
		if (equipSlots[i].id == id) {
			...
			equipSlots[i].set_unit(id, !unit_is_unusable(id));
			break;
		}
