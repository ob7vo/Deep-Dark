...
enum class AugmentType : size_t {
	...
	REINFORCE = 1 << 9, // Decreases damage taken by x% when under y% hp
	SCOPE = 1 << 10, // Increases sight and attack range by x% after going under y% hp
	...
	VOID = 1 << 16, // x% chance to deal y% of enemies max HP on hit(*)
	...
	DROP_BOX = 1ULL << 35, // When spawning, start at x% of the lanes distance
	TERMINATE = 1ULL << 36, // Enemies that are hit with x% health remaining will instantly die 
	LIGHTWEIGHT = 1ULL << 37, // Received knockback force is 50% greater (-)
	HEAVYWEIGHT = 1ULL << 38, // Received knockback force is 30% less (-)
	BULLY = 1ULL << 39, // Unit will Knockback enemies 50% father (*) (-)
	SALVAGE = 1ULL << 40, // Killing # of enemies will summon a Unit
	TRANSFORM = 1ULL << 41, // When an enemy(boss) dies, they transform into their next phase (-)
	SELF_DESTRUCT = 1ULL << 42, // Units releases a damaging explosion after death )-)
	FRAGILE = 1ULL << 43, // Unit dies after attacking (-)
	PROJECTILE = 1ULL << 44, // Launches a projectile when attacking,
	ROUGH = 1ULL << 45, // Projectiles that hit the unit will take extra chip dmg (-)
	LINK = 1ULL << 47, // When hit with a status, negative statuses wil be applied to enemies and positive statuses will be applied to allies. (-)
	SYPHON = 1ULL << 48, // Heals up x knockback threshold after killing y enemies
	DISCHARGE = 1ULL << 49  // Every x seconds, the damage taken over that time period is released as an explosion. (Exclusive to Capcitor types)
};
