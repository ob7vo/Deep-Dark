...
int UnitStatus::calculate_damage_and_effects(Unit& hitUnit, Unit& attacker) {
	// called from attacked Unit, parameters are from the attackING unit.
	...
	if (attacker.targeted_by_unit(hitUnit))
		dmg = (int)(dmg * calculate_damage_reduction(hitUnit.stats->augments));
...
int UnitStatus::apply_effects(Unit& hitUnit, const std::vector<Augment>& augments, 
...
bool UnitStatus::can_proc_status(Unit& unit, Augment augment, int hitIndex) const {
...
bool UnitStatus::take_damage(Unit& hitUnit, Unit& attacker) {
...
bool UnitStatus::take_damage(Unit& hitUnit, Surge& surge) {
