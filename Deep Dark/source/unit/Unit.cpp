...
// Checks
bool Unit::base_in_range(float sightDist) const{
...
bool Unit::can_teleport() const { return !stats->ancient_type() && stage->can_teleport(get_pos(), get_lane(), stats->team); }
...
void Unit::knockback_state(float deltaTime) {
	...
	if (can_teleport())
		...
		if (status.dead()) {
			std::cout << "died after knockback" << std::endl;
			destroy_unit();
		}
		else if (enemy_is_in_sight_range())
			anim.start_idle_or_attack_animation(*this);
...
void Unit::falling_state(float deltaTime) {
	...
	if (!movement.tweening() && events & FINAL_FRAME) {
		if (status.dead()) {
			std::cout << "died after falling" << std::endl;
			destroy_unit();
		}
		else if (enemy_is_in_sight_range())
			anim.start_idle_or_attack_animation(*this);
