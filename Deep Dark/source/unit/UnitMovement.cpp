...
void UnitMovement::teleport(Unit& unit) {
	auto tp = unit.stage->lanes[currentLane].get_teleporter(unit.stats->team);
	if (!tp || !unit.stage->can_push_move_request(unit.id)) return;
...
bool UnitMovement::try_leap(Unit& unit) {
	for (auto const [leftEdge, rightEdge] : unit.stage->get_lane(currentLane).gaps) {
		const float leapRange = unit.stats->get_augment(LEAP).value;
...
bool UnitMovement::try_push_jump_request(Unit& unit) {
