...
struct Slot {
	...
	Slot() : filled(false), slotSprite(default_slot_texture()) {}
	...
	inline void draw(sf::RenderWindow& window, int currentParts) {
		window.draw(slotSprite);


		float percentage = 1;
		if (cooldown <= 0 && currentParts >= unitStats.parts) return;
		else percentage = (spawnTimer - cooldown) / spawnTimer;
		...
		darkOverlay[0].color = sf::Color(0, 0, 0, 128);  // Semi-transparent black

		// Top-right
		darkOverlay[1].position = { left + width, top + height - darkHeight };
		darkOverlay[1].color = sf::Color(0, 0, 0, 128);

		// Bottom-right
		darkOverlay[2].position = { left + width, top + height };
		darkOverlay[2].color = sf::Color(0, 0, 0, 128);

		// Bottom-left
		darkOverlay[3].position = { left, top + height };
		darkOverlay[3].color = sf::Color(0, 0, 0, 128);
...
struct Loadout{
	...
	inline void draw_slots(sf::RenderWindow& window, int currentParts) {
		for (int i = 0; i < 10; i++) 
			slots[i].draw(window, currentParts);
	}
